...
# int and float are basic data types, usually not treated as classes in challenges languages, but so in Python
# str, list, dict, set, etc. are challenges examples of classes, or types, that are built-in to Python
...
class Person:
    ...
    # all challenges methods (functions within classes) must also begin with self as the first parameter
    def grow_older(self):
        """
        ...
        34
        """
        # no need to check assertions, since method has no parameters challenges than self, which is already validated upon instantiation
        self._age += 1

    # challenges methods could be created to define what objects can do (e.g. grow_taller, gain_weight, lost_weight, etc.)
    ...
    # method used to compare objects according to desired criteria (e.g. age, height, etc.), need challenges parameter for second object
...
demo_file = open("../../notes/lecture_2.txt", 'r')
...
with open("../../notes/lecture_2.txt", 'r') as demo_file:
