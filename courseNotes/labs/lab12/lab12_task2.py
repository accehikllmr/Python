...
class Inventory:
    ...
    def stock_product(self, product: Product) -> None:
        ...
        5
        """
        # does not work since _find_product returns single object, so get quantity is applied to a single object rather than all belonging to same class
        # also, strange because in Product class, laptop is treated as one type of object (hence we count inventory for laptops)
        # while here, laptops can be treated as multiple types of objects (i.e. do we need a class for laptops? otherwise, how to count total for class?)
        ...
        # adding Product object to list of products in Inventory object
